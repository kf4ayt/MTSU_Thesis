...
# runs the MPI-CPU_FW program 5 times, catching the output in a file whose name
...
for i in list(range(a)):
    ...
    while j <= int(sys.argv[3]):
        ...
        batchfile += "\n"
        batchfile += "mpirun ./MPI-CPU_FW " + graphList[i] + " no_save no_console no_check\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun ./MPI-CPU_FW " + graphList[i] + " no_save no_console no_check\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun ./MPI-CPU_FW " + graphList[i] + " no_save no_console no_check\n"
