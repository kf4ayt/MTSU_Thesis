...
if (len(sys.argv) != 5):
    print("Correct format is <program name> <input filename> <output file name> <threads/block> <num of nodes>")
    sys.exit()
...
for i in list(range(a)):
    ...
    while (j <= int(sys.argv[4])):
        ...
        for k in range(5):
            ...
            # Below line is what was required for it to run on AWS
            result = subprocess.run(['mpiexec', '--mca', 'btl_base_warn_component_unused', '0', '-n', str(j), './MPI-GPU_BF', graphfilename, 'short', 'no_print', 'no_console', 'no_check', tpb, 'multi'], stdout=subprocess.PIPE).stdout.decode('utf-8')
