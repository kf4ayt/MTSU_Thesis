...
for i in list(range(a)):
    ...
    while (j <= int(sys.argv[3])):
        ...
        for k in range(5):
            ...
            result = subprocess.run(['./SSSP_BF', graphfilename, 'short', 'no_print', 'no_console', '512', graph_operation], stdout=subprocess.PIPE).stdout.decode('utf-8')
