...
for i in list(range(a)):
    ...
    while (j <= int(sys.argv[3])):
        ...
        for k in range(5):
            ...
            # This somewhat complicated mpiexec line is what was required for it
            # to run on the AWS EC2 cluster.
            #
            result = subprocess.run(['mpiexec', '--mca', 'btl_base_warn_component_unused', '0', '-n', str(j), './MPI-CPU_BF', graphfilename, 'short', 'no_print', 'no_console', 'no_check'], stdout=subprocess.PIPE).stdout.decode('utf-8')
