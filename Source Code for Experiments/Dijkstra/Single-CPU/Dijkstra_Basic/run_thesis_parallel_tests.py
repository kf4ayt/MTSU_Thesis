...
for i in list(range(a)):
    ...
    while (j <= int(sys.argv[3])):
        ...
        for k in range(5):
            ...
            ## Change 'SSSP' to 'APSP' for APSP
            ##    
            result = subprocess.run(['./Basic_Dijkstra', graphfilename, 'SSSP', 'no_print_path', '512', 'no_console', 'no_error_check'], stdout=subprocess.PIPE).stdout.decode('utf-8')
