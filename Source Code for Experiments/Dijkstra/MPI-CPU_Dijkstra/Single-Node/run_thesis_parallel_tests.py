...
for i in list(range(a)):
    ...
    while (j <= int(sys.argv[3])):
        ...
        for k in range(5):
            ...
            result = subprocess.run(['mpiexec', '--mca', 'btl_base_warn_component_unused', '0', '-n', str(j), './MPI-CPU_Dijkstra', graphfilename, 'no_print', 'no_console', 'no_check'], stdout=subprocess.PIPE).stdout.decode('utf-8')
