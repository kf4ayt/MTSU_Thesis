...
for i in list(range(a)):
    ...
    while j <= int(sys.argv[3]):
        ...
        batchfile += "\n"
        batchfile += "mpirun MPI-CPU_Dijkstra " + graphList[i] + " no_print no_console no_check\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun MPI-CPU_Dijkstra " + graphList[i] + " no_print no_console no_check\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun MPI-CPU_Dijkstra " + graphList[i] + " no_print no_console no_check\n"
        ...
        # Now we need to spin our wheels and wait for the job to finish. So, every 15 seconds,
