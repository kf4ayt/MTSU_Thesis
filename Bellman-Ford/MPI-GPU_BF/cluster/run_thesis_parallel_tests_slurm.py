...
#   ...
#
# That file will have a filename of X-vertices_degree-Y_times.txt
...
if (len(sys.argv) != 5):
    print("Correct format is <program name> <input filename> <results dir> <threads/block> <num of nodes>")
    sys.exit()
...
for i in list(range(a)):
    ...
    results_dir = sys.argv[2]    # threads/block
    ...
    while j <= int(sys.argv[4]):
        ...
        batchfile += "#SBATCH --output=" + results_dir + "/" + graphName + "_" + tpb + "_" + str(j) + ".txt\n"
        batchfile += "\n"
        batchfile += "mpirun MPI-GPU_BF " + graphList[i] + " short no_print no_console no_check " + tpb + "\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun MPI-GPU_BF " + graphList[i] + " short no_print no_console no_check " + tpb + "\n"
        batchfile += "echo \"\"\n"
        batchfile += "mpirun MPI-GPU_BF " + graphList[i] + " short no_print no_console no_check " + tpb + "\n"
